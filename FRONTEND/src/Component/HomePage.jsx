...
const HomePage = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      // Refresh AOS after tasks render
      setTimeout(() => {
        AOS.refreshHard();
      }, [tasks]);
    } catch (error) {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {confirmPopup.show && (
        ...
        >
          ...
          <div className="flex gap-3 justify-center">
            <button
              onClick={() => {
                handleDeleteTask(confirmPopup.taskId);
      ...
      {editPopup.show && (
        ...
        >
          <h2 className="text-lg font-semibold mb-4 tracking-wide text-purple-300">
            Edit Task
          </h2>
          ...
          <input
            type="text"
            placeholder="Title"
            className="w-full px-4 py-2 mb-3 rounded-md bg-white/10 border border-white/20 text-white placeholder-white/40
              focus:outline-none focus:border-purple-400 transition-all font-light"
            value={editTitle}
          ...
          <textarea
            ...
            className="w-full px-4 py-2 mb-3 rounded-md bg-white/10 border border-white/20 text-white placeholder-white/40
              focus:outline-none focus:border-purple-400 transition-all resize-none font-light"
      ...
      <div className="w-full h-full grid grid-cols-4 gap-[15px] justify-items-start">
        {tasks.length === 0 && (
          <div className="col-start-2 col-span-2 flex items-center justify-center w-full h-[80vh]">
            <div
              data-aos="zoom-in"
            ...
            >
              ...
              <h2 className="text-white text-3xl font-bold">No Tasks Found!</h2>
              <p className="text-white/70 text-lg max-w-xs">
                You haven't added any tasks yet. Add tasks to manage your work
                efficiently.
              </p>
              <button
                className="mt-4 px-6 py-2 bg-purple-500/70 hover:bg-purple-500 text-white rounded-lg transition-all"
                onClick={() => navigator("/addTask")}
              >
                Add New Task
              </button>
            </div>
